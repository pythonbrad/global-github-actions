...
# disqualify participants with total_score=0
compute_total_score(){
	jq -s --argjson total_price 30000 'map((.correct_answers | map(.)) as $answers | (.user_answers // []) | map(.score=(.options_ids | map(select($answers[.] == true)) | length)-(.options_ids | map(select($answers[.] == false)) | length) | del(.options_ids))) | add | group_by(.first_name) | map((map(.score) | add) as $total_score | .[0] | del(.score) | .total_score=$total_score) | map(select(.total_score > 0)) | group_by(-.total_score) | to_entries | map(.price=$total_price/((.key+1)*2))' $@
}
